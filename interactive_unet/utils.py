...
def load_dataset(annotations=False):
    ...
    if len(image_volume_files) > 0:
        dataset = [
            volumedata.VolumeData(f, annotations=annotations)
...
def build_annotation_volumes(dataset):
    for i in range(len(dataset)):
        print(
            f"{i}/{len(dataset)} - Rebuilding annotation volumes for {dataset[i].filename}"
        )
...
def save_sample(
    ...
    train_weight_slice = weight_slice  # weight_slice * noise
    ...
    imsave(f"data/train/weights/{slice_idx:04d}.tiff", train_weight_slice)
...
def colored_to_categorical(colored_mask, include_background=True):
    ...
    mask = (
        np.stack(
            [
                np.all(colored_mask == color, axis=-1).astype(np.uint8)
...
def colored_to_class(colored_mask):
    ...
    mask = np.zeros(
        (categorical_mask.shape[0], categorical_mask.shape[1]), dtype="uint8"
    )
...
def class_to_categorical(class_mask, num_classes, weight=None):
    ...
    categorical_mask = np.zeros(
        (class_mask.shape[0], class_mask.shape[1], num_classes), dtype="uint8"
    )
